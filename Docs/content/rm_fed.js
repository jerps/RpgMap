...
<p>\
Call procedure <code>proc</code> for each key/item pair in <code>map</code>, injecting a \
pointer, then return the pointer that was returned with the last call \
to <code>proc</code>.<br>\
Procedure <code>proc</code> must have the following signature:\
</p>\
<pre class="procsig">\
  rtn type: pointer<br>\
...
<p>\
Procedure <code>proc</code> is executed for each key/item pair in <code>map</code>, starting \
with the lowest key, up to the highest key, or until <code>proc</code> sets output \
parameter <code>stop</code> to *on.<br>\
Use <a href="#rm_fedr"><code>rm_fedr</code></a> to start with the highest key down to the lowest key.<br>\
Each time <code>proc</code> is executed the key and item are passed as arguments <code>key</code> \
...
Parameter <code>usrd</code> is optional; if not passed the user data pointer is <code>*null</code>.<br>\
If no key/item pair is read (and <code>proc</code> doesn\'t get called) the return \
value will be <code>usrd</code>, or <code>*null</code> if <code>usrd</code> is not passed.<br>\
If <code>key1</code> and/or <code>key2</code> are passed then the key/item pairs that are \
...
These procedures dispose <code>key1</code> and <code>key2</code> automatically, unless \
