...
<p>\
Like <a href="#rm_get"><code>rm_get</code></a>, but procedures <code>rm__??</code> retrieve an item using one or more \
...
nested maps.<br>\
First the item in <code>map</code> with key equal to the first passed key is retrieved. \
...
Up to 10 keys can be passed. Parameters <code>k01</code> through <code>k10</code> are all \
optional. If none are passed then <code>*null</code> is returned. Keys that are <code>*null</code> \
are ignored.<br>\
Use procedure <a href="#rm__"><code>rm__</code></a> as a generic procedure which accepts keys of any type. \
...
passed.<br>\
If there are multiple keys equal to <code>key</code> then the first one is selected.<br>\
Procedure <code>rm__</code> disposes all passed keys automatically, unless a key is \
